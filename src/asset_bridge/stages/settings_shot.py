"""Stage 4: Lifestyle / settings shot generation.
...
class SettingsShotStage(Stage):
    ...
    async def run(
        ...
        *,
        ...
        settings_dir = output_dir / sku_id / "settings"
        settings_dir.mkdir(parents=True, exist_ok=True)

        colors: list[str] = config.get("metal_colors", ["yellow_gold", "white_gold", "rose_gold"])
        template_prompt = (prompt_template or {}).get("prompt", "Create a lifestyle photo featuring this necklace in {metal_color}.")
        ...
        if references_dir:
            ...
            ref_paths = find_reference_images(references_dir, "settings")[:2]
        ...
        if method == "lora_local":
            ...
            if not adapter.exists():
                result.errors.append(f"LoRA weights not found at {wp} — run `asset-bridge train-lora --stage settings`")
                return result
            lora_weights = wp
        ...
        for color in colors:
            ...
            if not source_images:
                result.errors.append(f"No source images for settings shot ({color})")
                continue
            ...
            try:
                ...
                if method == "lora_local":
                    logger.info("Settings shot [lora_local]: %s in %s", sku_id, color)
                    lora_prompt = f"{trigger_word}, {prompt}"
                    generated = await asyncio.to_thread(
                        self._generate_lora, lora_prompt, lora_weights
                    )
                    out_path = settings_dir / f"{color}_setting.png"
                    save_image(generated, out_path)
                    result.output_paths.append(out_path)
                else:
                    logger.info("Settings shot [api]: %s in %s", sku_id, color)
                ...
                logger.exception("Settings shot failed: %s %s", sku_id, color)
                result.errors.append(f"settings_{color}: {exc}")
